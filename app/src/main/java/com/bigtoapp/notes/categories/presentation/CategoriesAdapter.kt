...
class CategoriesViewHolder(
    ...
    fun bind(model: CategoryUi){
        model.map(mapper)
        viewCard.setOnClickListener { categoryActions.edit(model.map(mapId), model.map(mapColor)) }
    }
}
